...
type ProfessionalRowActionsProps = {
  id: string;
  name: string;
  isActive: boolean;
...
export function ProfessionalRowActions({
  ...
  function toggleActive() {
    ...
    startTransition(async () => {
      const result = await setProfessionalActiveAction(id, !isActive);
      if (!result.ok) {
      ...
      {canDelete ? (
        ...
        >
          <p className="text-sm leading-relaxed text-muted">
            ¿Eliminar a <span className="font-semibold text-navy">{name}</span>? Esta acción
